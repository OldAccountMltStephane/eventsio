...
<div class="container bg-event border rounded-0 mb-4 text-center text-sm-left">
    <div class="btn-group">
        <a href="{{ route('events.showEventEdition', $Evenement->id) }}" class="btn btn-event rounded-0">Editer
            l'événement</a>
        <a href="#" onClick="$('#deleteEvent').submit()" class="btn btn-event rounded-0">Supprimer l'événement</a>

        <form id="deleteEvent" action="{{ route('events.deleteEvent', $Evenement->id) }}" method="POST"
            style="display: none;">
            @csrf
            {{ method_field('DELETE') }}
        </form>
...
<div id="Sessions" class="container bg-white border rounded-0 p-3 text-center text-sm-left">
    ...
    <div session-id="{{ $Session->id }}" class="row mb-4 mb-sm-0">
        <div class="col-lg-2 px-3 py-2">
            <button onclick="$('#joinSession_{{ $Session->id }}').submit()"
                class="btn btn-sm btn-event rounded-0 w-100 @guest disabled @endguest" @guest disabled
                @endguest>Participer</button>
        </div>
        <div evenement-id="{{ $Evenement->id }}" session-id="{{ $Session->id }}"
            class="col-lg-4 px-3 py-2 my-auto align-middle sessionItem">{{ $Session->name }}</div>
        <div evenement-id="{{ $Evenement->id }}" session-id="{{ $Session->id }}"
            class="col-lg-3 px-3 py-2 my-auto align-middle sessionItem">Le
            {{ date('d/m/Y', strtotime($Session->start_at)) }} à {{ date('H:i', strtotime($Session->start_at)) }}</div>
        <div evenement-id="{{ $Evenement->id }}" session-id="{{ $Session->id }}"
            class="col-lg-3 px-3 py-2 my-auto align-middle sessionItem">Le
            {{ date('d/m/Y', strtotime($Session->end_at)) }} à {{ date('H:i', strtotime($Session->end_at)) }}</div>
    </div>

    <form id="joinSession_{{ $Session->id }}" action="{{ route('events.joinSession', [$Evenement->id, $Session->id])}}"
